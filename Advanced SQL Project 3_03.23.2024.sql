...
where COLUMN_NAME  like '%Purchase%'
order by Table_name
...
--4. Employee Performance Evaluation:
	...
	--4.2. Write SQL queries to aggregate sales data by employee, 
		 ...
		 select sub2.* , RANK() over(order by sub2.[total sales] desc) [Ranking]
		 from 
(		select sub.EmployeeKey, sub.[Full Name], SUM(sub.SalesAmount)  [Total sales]
		from (
		Select em.EmployeeKey, CONCAT(FirstName, ' ' , LastName) [Full Name] , SalesAmount
		...
		select sub2.*,      AVG([Total Deal closed]) over (order by [Running Sum of Orders]) [Running Average]
		from (
		select sub1.EmployeeKey,sub1.[Total Deal closed] , RANK() over (order by [Total Deal Closed] desc) [Ranking]  , 
		SUM([Total Deal closed]) over (order by [Total Deal Closed]) [Running Sum of Orders]	         
		from (
		select  EmployeeKey, COUNT(SalesOrderNumber) [Total Deal closed]
		from [dbo].[FactResellerSales]
		group by EmployeeKey ) sub1
		group by EmployeeKey, [Total Deal closed] ) sub2
